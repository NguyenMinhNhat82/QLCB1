...
from QuanLyChuyenBay.models import MayBay, ChuyenBay, LichChuyenBay, SanBay, User, Ticket,UserRole,TicketDetail,Customer,san_bay_trung_gian
...
def load_lich_may_bay(Tu=None, Den=None,Ngay = None):
    ...
    if Tu:
        for sanbay in sb:
            if sanbay.dia_diem.lower().__eq__(Tu.lower()):
                for C in cb:
    ...
    if Den:
        for sanbay in sb:
            if sanbay.dia_diem.lower().__eq__(Den.lower()):
                for C in cb:
